...
/*
// Generates a random date within the last X days
...
/**
 * Deletes all documents from a collection in batches of 500.
 * @param {admin.firestore.Firestore} db Firestore instance.
 * @param {string} collectionPath Name of the collection to delete.
 * @param {number} batchSize Batch delete size (max 500).
 */
...
async function deleteQueryBatch(db, query, resolve) {
    ...
    // Recursion to ensure all batches are deleted
    process.nextTick(() => {
...
const BASE_DATA = {
    ...
    // Random image service with fixed dimensions
...
/**
 * 1. Generates X random suppliers.
...
/**
 * 2. Generates X random warehouses.
 * @returns {Array<Object>} List of warehouses.
 */
async function seedWarehouses(db) {
    ...
    for (let i = 0; i < SEED_CONFIG.WAREHOUSES; i++) {
        ...
        const warehouse = {
            ...
            country: country,
            unitPrice: parseFloat((Math.random() * 40 + 10).toFixed(2)), // Price between 10 and 50
            createdAt: Timestamp.now(),
        };
...
/**
 * 4. Generates X products and Y stock documents simultaneously.
 */
async function seedProductsAndStock(db, suppliers, warehouses) {
    ...
    for (let i = 0; i < SEED_CONFIG.PRODUCTS; i++) {
        ...
        const ean = (Math.floor(Math.random() * 9000000000000) + 1000000000000).toString(); // 13-digit EAN

        // Random image URL using SKU as seed
        const photoUrl = `${BASE_DATA.RANDOM_IMAGE_URL_BASE}${sku}/300/300`;

        // 1. Create supplier list for the product
        const numSuppliers = Math.floor(Math.random() * 3) + 1; // 1 to 3 suppliers
        const supplierList = [];
        ...
        for (let j = 0; j < numSuppliers; j++) {
            ...
            // Assign 1 to 2 warehouses to this supplier
            ...
            for (let k = 0; k < numWarehouses; k++) {
                ...
                // 2. Create Stock document for this product/warehouse
...
async function seedOrders(db, clients) {
    ...
    // Fetch stock data to create realistic orders
    ...
    for (let i = 0; i < SEED_CONFIG.ORDERS; i++) {
        ...
        const numItems = Math.floor(Math.random() * 4) + 1; // 1 to 4 products per order
        const eanList = {};

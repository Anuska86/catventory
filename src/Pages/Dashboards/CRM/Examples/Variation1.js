...
const data55 = [
  { name: "Page A", total: 4000, pv: 2400, amt: 2400 },
  { name: "Page B", total: 3000, pv: 1398, amt: 2210 },
  { name: "Page C", total: 2000, pv: 9800, amt: 2290 },
  { name: "Page D", total: 2780, pv: 3908, amt: 2000 },
  { name: "Page E", total: 1890, pv: 4800, amt: 2181 },
  { name: "Page F", total: 2390, pv: 3800, amt: 2500 },
  { name: "Page G", total: 3490, pv: 4300, amt: 2100 },
  { name: "Page C", total: 2000, pv: 6800, amt: 2290 },
  { name: "Page D", total: 4780, pv: 7908, amt: 2000 },
  { name: "Page E", total: 2890, pv: 9800, amt: 2181 },
  { name: "Page F", total: 1390, pv: 3800, amt: 1500 },
  { name: "Page G", total: 3490, pv: 4300, amt: 2100 },
];
...
export default class CRMDashboard1 extends Component {
  ...
  countOrders() {
    const fetchOrders = async () => {
      try {
        // 1. Obtener todos los documentos de la colección "orders"
        const ordersSnapshot = await getDocs(collection(db, "orders"));

        // Si no hay órdenes, terminamos la ejecución
        if (ordersSnapshot.empty) {
          this.setState({
            orders: [],
            totalOrders: 0,
            totalClients: 0,
            totalCurrentMonthTotalSalesAggregateRounded: 0,
            loading: false
          });
        ...
        // 2. Mapear cada documento de la orden y realizar las consultas adicionales
        const ordersWithDetailsPromises = ordersSnapshot.docs.map(async (orderDoc) => {
          const orderData = { id: orderDoc.id, ...orderDoc.data() };

          // 3. Consulta para obtener el nombre del cliente
          let clientName = 'Unknown';
          if (orderData.clientId) {
            const clientsRef = collection(db, "clients");
            const clientQuery = query(clientsRef, where("clientId", "==", orderData.clientId));
            const clientSnapshot = await getDocs(clientQuery);

            if (!clientSnapshot.empty) {
              clientName = clientSnapshot.docs[0].data().name;
            }
          }

          // 4. Consulta para obtener el precio del producto
          let unitPrice = null;
          if (orderData.sku) {
            const productsRef = collection(db, "products");
            const productQuery = query(productsRef, where("sku", "==", orderData.sku));
            const productSnapshot = await getDocs(productQuery);

            if (!productSnapshot.empty) {
              unitPrice = productSnapshot.docs[0].data().unitPrice;
            }
          }

          // 5. Devolver un nuevo objeto con los datos combinados
          return {
            ...orderData,
            clientName,
            unitPrice,
        ...
        // 6. Esperar a que todas las promesas se resuelvan
        const ordersWithDetails = await Promise.all(ordersWithDetailsPromises);

        // 7. Contar el número de órdenes después de obtener los detalles
        const totalOrdersCount = ordersWithDetails.length;
        const totalClientsCount = new Set(ordersWithDetails.map(order => order.clientName)).size;
        const currentMonthTotalSalesAggregate = ordersWithDetails.reduce((acc, order) => acc + (order.unitPrice || 0) * (order.quantity || 1), 0);
        const currentMonthTotalSalesAggregateRounded = parseFloat(currentMonthTotalSalesAggregate.toFixed(2));
        // 8. Actualizar el estado con los datos completos y el conteo
        this.setState({
          orders: ordersWithDetails,
          totalOrders: totalOrdersCount,
          totalClients: totalClientsCount,
          totalCurrentMonthTotalSalesAggregateRounded: currentMonthTotalSalesAggregateRounded,
          loading: false, // <-- Asegúrate de manejar el estado de carga
        });
          ...
          <Row>
              ...
              <div className="card mb-3 widget-content bg-night-fade">
                <div className="widget-content-wrapper text-white">
                  <div className="widget-content-left">
                    ...
                    <div className="widget-subheading">Active Orders</div>
                  </div>
                  <div className="widget-content-right">
                    <div className="widget-numbers text-white">
                      <CountUp end={this.state.totalOrders} duration="2" />
                    </div>
                  </div>
                </div>
              </div>
            ...
            <Col md="6" xl="4">
              <div className="card mb-3 widget-content bg-arielle-smile">
                <div className="widget-content-wrapper text-white">
                  <div className="widget-content-left">
                    <div className="widget-heading">Total Clients</div>
                  ...
                  <div className="widget-content-right">
                    <div className="widget-numbers text-white">
                      <CountUp end={this.state.totalClients} separator="" decimals={0}
                        decimal="," duration="3" />
                    </div>
                  </div>
                </div>
            ...
            <Col md="6" xl="4">
              <div className="card mb-3 widget-content bg-happy-green">
                <div className="widget-content-wrapper text-white">
                  <div className="widget-content-left">
                    <div className="widget-heading">Active agreements</div>
                    <div className="widget-subheading">Status Active</div>
                  </div>
                  <div className="widget-content-right">
                    <div className="widget-numbers text-white">
                          ...
                          <div className="widget-chart-content pt-3 pe-3 ps-3">
                            <div className="widget-chart-flex">
                              <div className="widget-numbers">
                                <div className="widget-chart-flex">
                                  <div>
                                    <CountUp end={this.state.totalCurrentMonthTotalSalesAggregateRounded} separator="," decimals={2}
                                      decimal="," prefix="" duration="6" />
                                  </div>
                          ...
                          <div className="widget-chart-wrapper widget-chart-wrapper-lg opacity-10 m-0">
                            <ResponsiveContainer height="100%">
                              <AreaChart data={data55}
                                ...
                                <Tooltip />
                                <Area type="monotoneX" dataKey="total" strokeWidth={0} fill="#30b1ff" />
                              </AreaChart>
                            </ResponsiveContainer>
                          </div>
                        </div>
                          ...
                          <div className="widget-chart-content pt-3 pe-3 ps-3">
                            <div className="widget-chart-flex">
                              <div className="widget-numbers">
                                <div className="widget-chart-flex">
                                  <div>
                                    <CountUp start={0} end={851.34} separator="" decimals={2} decimal="," prefix="" duration="10" />
                                    <small className="opacity-5">€</small>
                                  </div>
                                  <div className="widget-title ms-2 opacity-5 font-size-lg text-muted">
                                    Total Sales
                    ...
                    <div className="widget-chart p-0">
                      <ResponsiveContainer height={330}>
                        <AreaChart data={data} margin={{ top: -25, right: 0, left: 0, bottom: 0 }}>
                          ...
                          <Tooltip />
                          <Area type="monotoneX" dataKey="total" stroke="#f7b924" strokeWidth={2}
                    ...
                    <div className="widget-chart p-0">
                      <ResponsiveContainer height={290}>
                        <AreaChart data={data2} margin={{ top: -25, right: 0, left: 0, bottom: 0 }}>
                          ...
                          <Tooltip />
                          <Area type="monotoneX" dataKey="total" stroke="var(--alternate)" strokeWidth={2}
                            fillOpacity={1} fill="url(#colorPv2)" />
                        </AreaChart>
                      </ResponsiveContainer>
                      <div className="widget-chart-content">
                    ...
                    <div className="widget-chart-wrapper widget-chart-wrapper-lg he-auto opacity-10 m-0">
                      <ResponsiveContainer height={131}>
                        <AreaChart data={data3} margin={{ top: -20, right: 0, left: 0, bottom: 0 }}>
                          <Tooltip />
                          <Area type="monotoneX" dataKey="total" stroke="#3ac47d" strokeWidth={3}
                    ...
                    <div className="widget-chart-wrapper widget-chart-wrapper-lg he-auto opacity-10 m-0">
                      <ResponsiveContainer height={131}>
                        <AreaChart data={data2} margin={{ top: -20, right: 0, left: 0, bottom: 0 }}>
                          <Tooltip />
                          <Area type="monotoneX" dataKey="total" stroke="#d92550"
                    ...
                    <div className="widget-chart-wrapper widget-chart-wrapper-lg he-auto opacity-10 m-0">
                      <ResponsiveContainer height={131}>
                        <AreaChart data={data} margin={{ top: -20, right: 0, left: 0, bottom: 0 }}>
                          <Tooltip />
                          <Area type="monotoneX" dataKey="total" stroke="rgba(255,255,255,.3)"

...
function getRandomTargetDate() {
  // Months in JS: 0=Jan, 1=Feb, 2=Mar...
 ...
 * @param {string} collectionPath Name of the collection to clear.
 * @param {number} batchSize Size of the deletion batch (max 500).
 */
...
async function deleteQueryBatch(db, query, resolve) {
  ...
  // Recurse to ensure all batches are deleted
  process.nextTick(() => {
...
const BASE_DATA = {
  ...
  // Service for random images using SKU as a seed
...
/**
 * 4. Generates X products and corresponding stock documents.
 */
async function seedProductsAndStock(db, suppliers, warehouses) {
  ...
  for (let i = 0; i < SEED_CONFIG.PRODUCTS; i++) {
    ...
    for (let j = 0; j < numSuppliers; j++) {
      ...
      for (let k = 0; k < numWarehouses; k++) {
        ...
        supplierWarehouses[
          warehouse.country.toLowerCase().replace(/\s/g, "_")
        ] = {
          name: warehouse.name,
          blocked_quantity: 0,
          quantity: stockQuantity,
          transport: BASE_DATA.TRANSPORT.slice(
            0,
    ...
    if ((productCount + stockCount) % SEED_CONFIG.BATCH_SIZE < 5) {
      await batch.commit();
      console.log(
        `💾 Committed products and stock batch. Total: ${productCount} products, ${stockCount} stock items.`,
      );
      batch = db.batch();
    }
  }
  ...
  await batch.commit();
  console.log(
    `🎉 Seeded ${productCount} products and ${stockCount} stock items.`,
  );
...
async function seedOrders(db, clients) {
  ...
  if (stockItems.length === 0) {
    console.warn(
      "🚨 No stock items found to create orders. Skipping order seeding.",
    );
    return 0;
  }
...
async function seedBatch(db) {
  ...
  const { productCount, stockCount } = await seedProductsAndStock(
    db,
    suppliers,
    warehouses,
  );

  // 3. Seed orders (uncommented to ensure they are created)
  const ordersCount = await seedOrders(db, clients);

  console.log("--- Final Counts ---");
  console.log(
    `✅ Suppliers: ${suppliers.length} (Target: ${SEED_CONFIG.SUPPLIERS})`,
  );
  console.log(`✅ Clients: ${clients.length} (Target: ${SEED_CONFIG.CLIENTS})`);
  console.log(
    `✅ Warehouses: ${warehouses.length} (Target: ${SEED_CONFIG.WAREHOUSES})`,
  );
  console.log(`✅ Products: ${productCount} (Target: ${SEED_CONFIG.PRODUCTS})`);
  console.log(
    `✅ Stock Items: ${stockCount} (Target: ~${SEED_CONFIG.PRODUCTS * 2})`,
  );

...
const PurchaseForecastComponent = () => {
  ...
  const handleSendAlgorithm = () => {
    ...
    try {
      setSubmissionStatus({
        status: "success",
        message: `${purchaseIds.length} orders sent to Algorithm.`,
      });
      ...
      const updatedData = data.map(item => ({
        ...
        algorithmStatus: (item.algorithmStatus === ALGORITHM_STATUS.PENDING && purchaseIds.includes(item.id))
  ...
  const handleEdit = (item) => { /* ... */ navigate(`/purchases/${item.id}`); };
  const handleRowSelected = useCallback((state) => { setSelectedRows(state.selectedRows); }, []);
  const handleDelete = (item) => { const updatedProducts = data.filter((p) => p.id !== item.id); setData(updatedProducts); };
  const handleSave = async (updatedItem) => { const updatedProducts = data.map((p) => p.id === updatedItem.id ? updatedItem : p); setData(updatedProducts); };
  ...
  const purchaseColumns = [
    ...
    { name: "Status Algo", selector: (row) => row.algorithmStatus, sortable: true, },
  ...
  useEffect(() => {
    const itemsToUpdate = data.filter(item => item.algorithmStatus === ALGORITHM_STATUS.IN_EXECUTION);
      ...
      const mockTimer = setTimeout(() => {
        setData(currentData =>
          currentData.map(item => ({
            ...
            algorithmStatus: item.algorithmStatus === ALGORITHM_STATUS.IN_EXECUTION
              ? ALGORITHM_STATUS.RESULTS
              : item.algorithmStatus,
          }))
        );
        ...
        setSubmissionStatus({
          status: "info",
          message: `Mock RabbitMQ: ${itemsToUpdate.length} orders finished execution.`,
        });
      }, 5000);
  ...
  const pendingCount = useMemo(() => data.filter(item => item.algorithmStatus === ALGORITHM_STATUS.PENDING).length, [data]);
  const inExecutionCount = useMemo(() => data.filter(item => item.algorithmStatus === ALGORITHM_STATUS.IN_EXECUTION).length, [data]);
  const resultsCount = useMemo(() => data.filter(item => item.algorithmStatus === ALGORITHM_STATUS.RESULTS).length, [data]);

  return (
    <Fragment>
      <PageTitle
        heading="Orders Forecast Status"
        subheading="Manage and track your orders forecast."
                ...
                <CardHeader className="p-0">
                  ...
                  <Nav tabs className="d-flex justify-content-between w-100">
                    <div className="d-flex" style={{ marginLeft:"1rem" }}>
                      ...
                      <NavItem>
                        <NavLink
                          className={classnames({ active: activeView === VIEWS.PENDING })}
                          onClick={() => { toggleView(VIEWS.PENDING); }}
                        >
                      ...
                      <NavItem>
                        <NavLink
                          className={classnames({ active: activeView === VIEWS.IN_EXECUTION })}
                          onClick={() => { toggleView(VIEWS.IN_EXECUTION); }}
                        >
                      ...
                      <NavItem>
                        <NavLink
                          className={classnames({ active: activeView === VIEWS.RESULTS })}
                          onClick={() => { toggleView(VIEWS.RESULTS); }}
                        >
                    ...
                    <div className="d-flex align-items-center p-2">
                      ...
                      {activeView === VIEWS.PENDING && (
                        <Button
                          ...
                          className="me-2"
                        >
                          Send Algorithm ({selectedRows.length})
                        </Button>
                      )}

                      {/* Botón Download/ExportCSV */}
                      <ExportCSV data={filteredData} fileName={`${activeView.toLowerCase()}_orders.csv`} />
                    </div>
                  </Nav>
                </CardHeader>
...
export default PurchaseForecastComponent;